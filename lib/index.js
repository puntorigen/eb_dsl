  ...
  class eb extends base_deploy {
    ...
    _createEBx_configNode() {
      ...
      return _asyncToGenerator(function* () {
        ...
        var data = {
          option_settings: {
            'aws:elasticbeanstalk:application:environment': {
              APP_PORT: _this2.context.x_state.central_config.port,
              CLUSTER: 1,
        ...
        if (_this2.context.x_state.central_config.instance_type) {
          data.option_settings.container_commands = {
            'aws:autoscaling:launchconfiguration': {
              InstanceType: _this2.context.x_state.central_config.instance_type
            }
          };
        } //port


        if (_this2.context.x_state.central_config.port != 8081) {
          data.container_commands = {
            ...
            '01_add_redirect_http': {
              command: "sudo iptables -t nat -A PREROUTING -i eth0 -p tcp --dport 80 -j REDIRECT --to-port ".concat(_this2.context.x_state.central_config.port)
            }
          };
        ...
        if (_this2.context.x_state.central_config.stage && _this2.context.x_state.central_config.stage != '') {
          data.option_settings['aws:elasticbeanstalk:application:environment'].STAGE = _this2.context.x_state.central_config.stage;

          if (_this2.context.x_state.central_config.stage != 'dev') {
            data.option_settings['aws:elasticbeanstalk:application:environment'].START_TYPE = _this2.context.x_state.central_config.stage;
          }
        }
    ...
    _createEBx_timeout() {
      ...
      return _asyncToGenerator(function* () {
        // create 01_confignode content for setting ENV vars within EB instance
        if (_this3.context.x_state.central_config.timeout) {
          var yaml = require('yaml');

          var data = {
            container_commands: {
              extend_proxy_timeout: {
                command: "sed -i '/s*location / {/c         client_max_body_size 500M;         location / {                 proxy_connect_timeout       ".concat(_this3.context.x_state.central_config.timeout, ";                proxy_send_timeout          ").concat(_this3.context.x_state.central_config.timeout, ";                proxy_read_timeout          ").concat(_this3.context.x_state.central_config.timeout, ";                send_timeout                ").concat(_this3.context.x_state.central_config.timeout, ";        ' /tmp/deployment/config/#etc#nginx#conf.d#00_elastic_beanstalk_proxy.conf")
              }
            }
          }; //write
    ...
    run() {
      ...
      return _asyncToGenerator(function* () {
        ...
        if (eb_appname != '') {
          ...
          if (_this6.context.x_state.central_config.rtc == true) {
            yield _this6._createEBx_sockets();
          }

          if (_this6.context.x_state.npm.puppeteer || _this6.x_state.context.npm['puppeteer-code']) {
