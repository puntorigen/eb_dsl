  ...
  class eb extends base_deploy {
    ...
    _createEBx_configEB() {
      ...
      return _asyncToGenerator(function* () {
        ...
        if (_this2.context.x_state.config_node.aws.region) {
          data.global.default_region = _this2.context.x_state.config_node.aws.region;
        } //write
        ...
        yield _this2.context.writeFile(path.join(eb_dir, 'config.yml'), yaml.stringify(data, {
          version: '1.1'
        }));
      })();
    }
    ...
    _createEBx_configNode() {
      ...
      return _asyncToGenerator(function* () {
        ...
        var data = {
          option_settings: {
            'aws:elasticbeanstalk:application:environment': {
              APP_PORT: _this3.context.x_state.central_config.port,
              CLUSTER: 1,
        ...
        if (_this3.context.x_state.central_config.instance_type) {
          data.option_settings.container_commands = {
            'aws:autoscaling:launchconfiguration': {
              InstanceType: _this3.context.x_state.central_config.instance_type
            }
          };
        } //port


        if (_this3.context.x_state.central_config.port != 8081) {
          data.container_commands = {
            ...
            '01_add_redirect_http': {
              command: "sudo iptables -t nat -A PREROUTING -i eth0 -p tcp --dport 80 -j REDIRECT --to-port ".concat(_this3.context.x_state.central_config.port)
            }
          };
        ...
        if (_this3.context.x_state.central_config.stage && _this3.context.x_state.central_config.stage != '') {
          data.option_settings['aws:elasticbeanstalk:application:environment'].STAGE = _this3.context.x_state.central_config.stage;

          if (_this3.context.x_state.central_config.stage != 'dev') {
            data.option_settings['aws:elasticbeanstalk:application:environment'].START_TYPE = _this3.context.x_state.central_config.stage;
          }
        }

        var _loop = function _loop(key) {
          // omit special config 'reserved' (aurora,vpc,aws) node keys
          if (!['copiar'].includes(key) && typeof _this3.context.x_state.config_node[key] === 'object') {
            Object.keys(_this3.context.x_state.config_node[key]).map(function (attr) {
        ...
        for (var key in _this3.context.x_state.config_node) {
          _loop(key);
        } //write
        ...
        yield _this3.context.writeFile(path.join(eb_dir, '01_confignode.config'), yaml.stringify(data, {
          version: '1.1'
        }));
      })();
    }
    ...
    _createEBx_timeout() {
      ...
      return _asyncToGenerator(function* () {
        // create 01_confignode content for setting ENV vars within EB instance
        if (_this4.context.x_state.central_config.timeout) {
          var yaml = require('yaml');

          var data = {
            container_commands: {
              extend_proxy_timeout: {
                command: "sed -i '/\\s*location \\/ {/c \\\n        client_max_body_size 500M; \\\n        location / { \\\n                proxy_connect_timeout       ".concat(_this4.context.x_state.central_config.timeout, ";\\\n                proxy_send_timeout          ").concat(_this4.context.x_state.central_config.timeout, ";\\\n                proxy_read_timeout          ").concat(_this4.context.x_state.central_config.timeout, ";\\\n                send_timeout                ").concat(_this4.context.x_state.central_config.timeout, ";\\\n        ' /tmp/deployment/config/#etc#nginx#conf.d#00_elastic_beanstalk_proxy.conf")
              }
            }
          }; //write
          ...
          var eb_dir = path.join(eb_base, '.ebextensions');
          yield _this4.context.writeFile(path.join(eb_dir, 'extend-proxy-timeout.config'), yaml.stringify(data)); //, { version:'1.1' }
        }
      })();
    }
    ...
    _createEBx_sockets() {
      ...
      return _asyncToGenerator(function* () {
        ...
        var eb_dir = path.join(eb_base, '.ebextensions');
        yield _this5.context.writeFile(path.join(eb_dir, 'enable-websockets.config'), yaml.stringify(data, {
          version: '1.1'
        }));
      })();
    }
    ...
    _createEBx_puppeteer() {
      ...
      return _asyncToGenerator(function* () {
        ...
        var eb_dir = path.join(eb_base, '.ebextensions');
        yield _this6.context.writeFile(path.join(eb_dir, 'puppeteer.config'), yaml.stringify(data, {
          version: '1.1'
        }));
      })();
    }
    ...
    run() {
      ...
      return _asyncToGenerator(function* () {
        ...
        _this7.context.debug('AWS EB deploy');

        var eb_full = _this7.context.x_state.central_config.deploy.replaceAll('eb:', '');
        ...
        if (eb_appname != '') {
          var spinner = _this7.context.x_console.spinner({
          ...
          yield _this7.context.writeFile(path.join(eb_base, '.npmrc'), 'unsafe-perm=true'); //write .ebextensions/config.yml

          yield _this7._createEBx_configEB(); //write .ebextensions/01_confignode.config

          yield _this7._createEBx_configNode(); //write .ebextensions/extend-proxy-timeout.config

          yield _this7._createEBx_timeout(); //enable websockets?

          if (_this7.context.x_state.central_config.rtc == true) {
            yield _this7._createEBx_sockets();
          }

          if (_this7.context.x_state.npm.puppeteer || _this7.context.x_state.npm['puppeteer-code']) {
            yield _this7._createEBx_puppeteer();
          } //create .ebignore file


          var eb_ig = "node_modules/\njspm_packages/\n.npm\n.node_repl_history\n*.tgz\n.yarn-integrity\n.editorconfig\n# Mac OSX\n.DS_Store\n# Elastic Beanstalk Files\n.elasticbeanstalk/*\n!.elasticbeanstalk/*.cfg.yml\n!.elasticbeanstalk/*.global.yml";
          yield _this7.context.writeFile(path.join(eb_base, '.ebignore'), eb_ig); //init git if not already

          spinner.succeed('EB config files created successfully');
          var results = {};
          var git_exists = yield _this7.context.exists(path.join(eb_base, '.git'));
          ...
          if (!git_exists) {
            ...
            yield _this7.context.writeFile(path.join(eb_base, '.gitignore'), git_ignore);
          ...
          try {
            if (_this7.context.x_config.nodeploy && _this7.context.x_config.nodeploy == true) {
              spinner.succeed('EB ready to be deployed (nodeploy as requested)');

              _this7.context.x_console.outT({
                message: "Aborting final deployment as requested",
                color: 'brightRed'
              });
            } else {
            ...
            if (gi.code == 4) {
              ...
              if (errors.length == 0) {
                ...
                try {
                  ...
                  yield _this7.launchTerminal('eb', ['create', eb_instance], eb_base);
                  yield _this7.sleep(1000);
          ...
          if (errors.length == 0 && _this7.context.x_state.central_config.debug == true && !_this7.context.x_config.nodeploy) {
    ...
    post() {
      ...
      return _asyncToGenerator(function* () {
        ...
        if (!_this8.context.x_state.central_config.componente && _this8.context.x_state.central_config.deploy && _this8.context.x_state.central_config.deploy.indexOf('eb:') != -1 && _this8.context.x_state.config_node.aws) {
          ...
          if (yield _this8.context.exists(aws_bak)) {
    ...
    pre() {
      ...
      return _asyncToGenerator(function* () {
        if (!_this9.context.x_state.central_config.componente && _this9.context.x_state.central_config.deploy && _this9.context.x_state.central_config.deploy.indexOf('eb:') != -1) {
          ...
          if (_this9.context.x_state.config_node.aws) {
            ...
            _this9.context.x_console.outT({
              message: "config:aws:access ->".concat(_this9.context.x_state.config_node.aws.access)
            });

            _this9.context.x_console.outT({
              message: "config:aws:secret ->".concat(_this9.context.x_state.config_node.aws.secret)
            }); // transform config_node.aws keys into ini


            var to_ini = ini.stringify({
              aws_access_key_id: _this9.context.x_state.config_node.aws.access,
              aws_secret_access_key: _this9.context.x_state.config_node.aws.secret
            }, {
              section: 'default'
            });

            _this9.context.debug('Setting .aws/credentials from config node'); // save as .aws/credentials (ini file)
            ...
            if (parsed.default) _this9.context.debug('Using local system AWS credentials', parsed.default);
            _this9.context.x_state.config_node.aws = {
              access: '',
              secret: ''
            };
            if (parsed.default.aws_access_key_id) _this9.context.x_state.config_node.aws.access = parsed.default.aws_access_key_id;
            if (parsed.default.aws_secret_access_key) _this9.context.x_state.config_node.aws.secret = parsed.default.aws_secret_access_key;
          }
        }
      })();
    } // config hooks
    ...
    codeForModel(model) {
      ...
      return _asyncToGenerator(function* () {
        ...
        if (_this10.context.x_state.npm['aws-sdk']) {
          if (!_this10.context.x_state.config_node.aws) {
            _this10.context.x_state.npm['aws-get-credentials'] = '*';
            resp.push("const AWS = require('aws-sdk');\n                (async function() {\n                    const { getAWSCredentials } = require('aws-get-credentials');\n                    AWS.config.credentials = await getAWSCredentials();;\n                })();\n                const AWS_s3 = new AWS.S3();");
          } else {
            var aws_data = {
              accessKeyId: _this10.context.x_state.config_node.aws.access,
              secretAccessKey: _this10.context.x_state.config_node.aws.secret
            };

            if (_this10.context.x_state.config_node.aws.region) {
              aws_data.region = _this10.context.x_state.config_node.aws.region;
            }

            resp.push("const AWS = require('aws-sdk');\n                AWS.config.update(".concat(_this10.context.jsDump(aws_data), ");\n                const AWS_s3 = new AWS.S3();"));
          }
        }
          ...
          for (var table of modelos[0].nodes) {
            ...
            if (fields[':index']) {
              ...
              resp.tables[table.text].indexes.push({
                name: yield _this22.hash(table.text + '_' + table[':index']),
                unique: false,
            ...
            if (fields[':index_unique']) {
              if (!resp.tables[table.text].indexes) resp.tables[table.text].indexes = [];
              resp.tables[table.text].indexes.push({
                name: yield _this22.hash(table.text + '_' + table[':index_unique']),
                unique: true,
    ...
    _writeModelos() {
      ...
      return _asyncToGenerator(function* () {
        ...
        for (var table in _this23.x_state.models.tables) {
          ...
          if (_this23.x_state.models.tables[table].indexes) {
            //write model with indexes
            var indexes = {
              indexes: _this23.x_state.models.tables[table].indexes
            };
            content = "module.exports = (sequelize, type) => {\n                    return sequelize.define('".concat(db_name, "', ").concat(_this23.jsDump(model, 'type.'), ", ").concat(_this23.jsDump(indexes), ");\n                }");
          } else {
